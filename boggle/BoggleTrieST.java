...
public class BoggleTrieST<Value> {
    private static final int R = 26;        // capital letters
    private static final char charStart = 'A';
   ...
   /**
    ...
    private void collect(Node x, StringBuilder prefix, Queue<String> results) {
        ...
        for (int c = 0; c < R; c++) {
            prefix.append(c + charStart);
            collect(x.next[c], prefix, results);
            prefix.deleteCharAt(prefix.length() - 1);
        }
    }

    public boolean hasChildren(String prefix) {
        // Iterate over non-null children
        // StdOut.println(prefix);
        Node curr = get(root, prefix, 0);
        if (curr == null) return false;
        // StdOut.println(hasChildren(curr));
        return hasChildren(curr);
    }

    private boolean hasChildren(Node curr) {
        for (int i = 0; i < curr.next.length; i++) {
            if (curr.next[i] == null) continue;
            // if any one of the children  has a non-null value, return true
            if (curr.next[i].val != null) return true;
            // if any one of the children is non-null and hasChildren, return true
            if (curr.next[i] != null && hasChildren(curr.next[i])) return true;
        }
        return false;
    }
